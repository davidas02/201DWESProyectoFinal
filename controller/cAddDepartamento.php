...
if (isset($_REQUEST['aceptar'])){//al hacer clic en el boton aceptar comprobaremos los campos y si son correctos modificaremos los valores por los escritos
    ...
    if(is_object(DepartamentoPDO::buscarDepartamentoPorCodigo($_REQUEST["codigo"]))){
        $aErrores['duplicado']="Ya existe un departamento con este codigo";
        $entradaOk = false;
    }
    ...
    DepartamentoPDO::addDepartamento($aRespuestas['codigo'], $aRespuestas['descripcion'],$aRespuestas['volumenNegocio']);
    $_SESSION['paginaEnCurso'] = $_SESSION['paginaAnterior'];
    header('Location: index.php');
    exit();
    }
